...
parser.add_argument("--supermarket", type=str, help="choose supermarket preset", required=True)
parser.add_argument("--CHROMEDRIVER_PATH", type=str, default="res/chromedriver.exe", help="path for chromedriver.exe")
parser.add_argument("--URL_PREFIX", type=str, default="", help="url prefix for the scraper")
parser.add_argument("--DATABASE", type=str, default="", help="database to connect to")
parser.add_argument("--MAX_CATEGORY_WORKERS", type=int, default=1, choices=range(1, 129), help="number of concurrent "
                                                                                               "categories")
parser.add_argument("--PRODUCTS_ON_PAGE", type=int, default=24, choices=range(1, 10000), help="number of products per "
                                                                                              "page")
parser.add_argument("--TOTAL_CATEGORIES", type=int, default=5, choices=range(1, 17), help="total number of categories")
...
if args.supermarket == "morrisons":
	if args.URL_PREFIX == "":
		args.URL_PREFIX = "https://groceries.morrisons.com"
	if args.DATABASE == "":
		args.DATABASE = "morrisons.db"
	if args.PRODUCTS_ON_PAGE == 24:
		args.PRODUCTS_ON_PAGE = 128  # TODO
	if args.TOTAL_CATEGORIES == 5:
		args.TOTAL_CATEGORIES = 18  # TODO
...
def create_category_scraper(driver, cat_url) -> ContinuousScraper:
	...
	if args.supermarket == "asda":
		...
		return Scrapers.morrisons.CategoryPageScraper(driver, cat_url, URL_PREFIX)
	elif args.supermarket == "sainsburys":
		pass
	else:
...
if args.supermarket == "asda":
	...
	cat_urls = home_scraper.scrape(URL_PREFIX + '/browse')
elif args.supermarket == "sainsburys":
	cat_urls = []
else:
